...
export default function RegistrationForm() {
  ...
  if (isSuccess) {
    return (
      <div className="bg-white rounded-xl shadow-md border border-gray-100 p-8 text-center max-w-2xl mx-auto my-12">
        <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6">
          <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
          </svg>
        </div>
        <h2 className="text-2xl font-bold text-[#1e3a5f] mb-2">Pendaftaran Berhasil!</h2>
        <p className="text-gray-600 mb-6">Pendaftaran Anda sedang diproses oleh petugas kami.</p>
        
        <div className="bg-gray-50 p-6 rounded-lg mb-6 border border-gray-200">
          <p className="text-sm text-gray-500 mb-1">Nomor Tiket Anda:</p>
          <p className="text-3xl font-bold text-[#1e3a5f] tracking-wider">{ticketNumber}</p>
        </div>
        
        <p className="text-sm text-gray-600 bg-blue-50 text-blue-800 p-4 rounded-lg border border-blue-100">
          Silakan tunggu konfirmasi selanjutnya yang akan dikirimkan melalui email Anda (<strong>{formData.email}</strong>).
        </p>
        <button 
          onClick={() => window.location.reload()}
          className="mt-8 px-6 py-2 bg-[#1e3a5f] text-white rounded-lg font-medium hover:bg-blue-900 transition-colors"
        >
          Daftar Anggota Baru
        </button>
      </div>
